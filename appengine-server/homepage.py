...
function livedata(){
	document.getElementById("dataspot").innerHTML = '<iframe src="/crowdbot/out?get=livedata" width="400" height="300" scrolling="yes" frameborder="0" style="border:0px none transparent;"></iframe>';
}
		</script>
	</head>
	<body onload="init()">
		<table><tr><td>
			...
			<br/>
			<div id="dataspot" style="width:400;height:300px;">
				<button onclick="livedata()">
					Show Live Data
				</button>
			<!--
		...
		<table><tr><td>
			<form accept-charset="UTF-8" action="/crowdbot" method="post" style="border-right:1px solid silver;margin-right:15px;padding-right:15px;">
				...
				<h3>
					...
					<select id='samples' href='#' onchange='writeSample()'>
						...
						<option value="servo">Motor Test</option>
			...
			<div style="margin-left:15px;">
				...
				<ul class="spacedlist" style="font-family:courier;list-style:none;">
					<li>pinMode( PIN# , INPUT or OUTPUT )<br/>prepares a pin to read or write information</li>
					<li>digitalWrite( PIN# , HIGH or LOW )<br/>turns the pin on or off / GND</li>
					<li style="padding-bottom:5px;border-bottom:1px solid silver;">delay( milliseconds )<br/>waits before continuing to the next line</li>
					
					<li>analogRead( PIN# )<br/>returns sensor value from 0 (off) to 1024 (full-on)</li>
					<!-- <li>analogWrite( PIN# , 0 <= val <= 255 )<br/>activates a voltage between 0 and 5V</li> -->
					<li style="padding-bottom:5px;border-bottom:1px solid silver;">digitalRead( PIN# )<br/>returns sensor value as HIGH or LOW</li>
					
					<li>#include &lt;Servo.h&gt;<br/>Servo myservo;<br/>paste at start of code to allow motor control</li>
					<li>myservo.attach(9);<br/>Prepare servo motor inside setup</li>
					<li style="padding-bottom:5px;border-bottom:1px solid silver;">myservo.write( 0 <= val <= 179 )<br/>Turn motor to 0-179 degrees</li>
...
Data returned by your sketch is stored at http://mapmeld.com/crowdbot/out?get=feed&id=""" + self.request.get('id')
		...
		if((self.request.get('dostream') == 'on') and (self.request.get('mysketch').find('Serial.begin') > -1)):
